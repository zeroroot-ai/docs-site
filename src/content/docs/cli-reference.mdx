...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--capability` | stringArray | — | Session capability to grant in the credential ceiling; repeatable. Reserved names: mission:originate (start its own mission), mission:delegate |
...
Scaffold, validate, register, and run Gibson components (agent | tool | plugin | connector)
...
Scaffold a new component directory (agent | tool | plugin | connector)
...
| `--kind` | string | — | component kind: agent \| tool \| plugin \| connector (required) |
...
Manage tenant connectors (ConnectorService)

```text
gibson connector — enable and manage connectors via
...
reconciles onto ToolHive. You do not write YAML. Authenticated as you: run `gibson login` first.
...
#### `gibson connector catalog`

List the curated connectors this tenant may enable

```text
Call ConnectorService.ListCatalog and print the curated
...
#### `gibson connector disable`

Disable an enabled connector

```text
Call ConnectorService.DisableConnector to disable an enabled
...
| `--tenant` | string | — | Override the active tenant id for this call. |
| `--timeout` | duration | `30s` | RPC deadline |
| `--yes` | bool | — | Do not ask to confirm the disable |

#### `gibson connector enable`

Enable a connector from the catalog

```text
Call ConnectorService.EnableConnector for a catalog entry
...
#### `gibson connector list`

List the tenant's enabled connectors and their live status

```text
Call ConnectorService.ListConnectors and print the tenant's
...
Initialize a Gibson workspace (writes .gibson/workspace.yaml)
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--detach` | bool | — | Return once the daemon has started the run and print the mission id; do not wait for the mission to finish |
