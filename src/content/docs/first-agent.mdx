...
This page shows how to write an agent from scratch with the Gibson
SDK.

## Prerequisites

- You completed [Install](/docs/install), and `gibson inspect` returns
...
model at dispatch time. See [Observability](/docs/observability) for
...
Leave the process running. In the Missions section of the dashboard,
add a node that points at the name of your agent. Click **Run**. The
mission detail page streams events as your agent works. See
[Missions](/docs/missions) for the full event taxonomy.

## A more realistic example

This bug-bounty triage agent uses a tool and the graph, and submits a
finding:
...
Their own pages explain tools, plugins, and the graph:
...
- **[Knowledge graph](/docs/knowledge-graph)**: query and write the
