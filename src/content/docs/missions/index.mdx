...
| `NODE_TYPE_JOB` | Opens a job on a bank of always-on Claude Code and runs the verify loop until the acceptance passes. In build. See [Job node](/docs/missions/job-node). |
...
| `reuse_policy` | ReusePolicy | Scope and re-use across runs, see [Reuse policy](#reuse-policy-skip--rerun--merge). |
