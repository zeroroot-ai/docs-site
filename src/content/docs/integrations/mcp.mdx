...
**Not yet published.** `@zeroroot-ai/gibson-mcp` is in build. Until the
package is on npm, the `npx` command below does not resolve. The
...
The plugin as a bundle around `@zeroroot-ai/gibson-mcp` is in build. Today
the plugin runs a Claude-only server with the same tools, and the install
commands above are the same.
...
A later release makes the plugin load the server. Today the plugin registers
...
host with a headless mode has a CI smoke test there. That directory is in
build.
